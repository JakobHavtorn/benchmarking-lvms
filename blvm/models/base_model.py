...
def load_model(path, model_class_name: str = None, device: torch.device = torch.device("cpu")):
    ...
    if model_class_name is None:
        if os.path.exists(os.path.join(path, MODEL_CLASS_NAME_STR)):
            ...
            LOGGER.debug(f"Loading '{model_class_name}' from 'blvm.models'")
        else:
            raise RuntimeError(f"Name of class of model to load was not given and not saved in checkpoint: {path}")

    model_class = getattr(blvm.models, model_class_name)
...
class BaseModel(nn.Module):
    ...
    def _get_init_arguments(self):
        ...
        if len(missing_names) > 0:
            msg = (
                ...
                f"Either they were never set or they might be objects of type `torch.nn.Module`."
            )
            LOGGER.warning(msg)
    ...
    def summary(
        self: nn.Module,
        input_size: Optional[Iterable[int]] = None,
